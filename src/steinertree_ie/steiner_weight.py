from steinertree_ie.simplified import simplified_problem
...
def steiner_count(weight:int, root:int, v: Vertex_collection, fft: bool):
    ...
    for subset_of_terminals in powerset(v.terminal_indicies):
        ...
        IE_count += (-1)**parity * simplified_problem(root, weight, v, subset_of_terminals, fft)
    
    return(IE_count)
    
def clever_search(f, root, v, fft, step = 5):
    # Step 1: Find upper bound where f(x, v) becomes nonzero
    x = 1
    while f(x, root, v, fft) == 0:
        x += step

    # Step 2: Binary search between previous zero and this nonzero
    
    for i in range(step):
        if f(x-step+1+i, root, v, fft) != 0:
            return(x-step+1+i)



def steiner(v: Vertex_collection, fft = False):
    root = v.terminal_indicies[0]

    # num_solutions = steiner_count(root, weight, v)
    
    return(clever_search(steiner_count, root, v, fft))
